...
if ($filterK) { $where[] = 'k.KID = ?'; $params[] = $filterK; }
if ($filterS) { $where[] = 's.SID = ?'; $params[] = $filterS; }

$sql = 'SELECT i.InvID, i.Barcode,
               g.G_Name AS Geraet, g.G_Hersteller AS Hersteller,
               g.G_Kaufdatum, g.G_Kosten, g.G_Garantieende,
...
<div class="page-header fade-in">
    ...
    <p class="page-header__sub">Alle erfassten Inventarartikel im Überblick.</p>
</div>
...
<div class="card fade-in">
    <div class="card__header">
        <span class="card__title">
            <?= $gesamt ?> <?= ($suche || $filterK || $filterS) ? 'Ergebnisse' : 'Einträge' ?>
            <?php if ($suche || $filterK || $filterS): ?>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr<?= $garantieAbgelaufen ? ' style="background:#FFF5F5;"' : '' ?>>
                    ...
                    <td><strong><?= htmlspecialchars($item['Geraet']) ?></strong></td>
                    <td style="color:var(--gray-3);"><?= htmlspecialchars($item['Hersteller'] ?? '–') ?></td>
                    ...
                    <td style="font-size:.85rem;"><?= $item['G_Kaufdatum'] ? date('d.m.Y', strtotime($item['G_Kaufdatum'])) : '<span style="color:var(--gray-4);">–</span>' ?></td>
